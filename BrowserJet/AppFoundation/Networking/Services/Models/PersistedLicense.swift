...
struct PersistedLicense: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        authenticationType = try c.decode(String.self, forKey: .authenticationType)
        userExpiryDate = try c.decode(Date.self, forKey: .userExpiryDate)
        numberOfLicenses = try c.decode(Int.self, forKey: .numberOfLicenses)
        proxyEnabled = try c.decode(Bool.self, forKey: .proxyEnabled)
        username = try c.decode(String.self, forKey: .username)
        proxyPackage = try c.decode(Int.self, forKey: .proxyPackage)
        userKind = try c.decode(String.self, forKey: .userKind)
        
        subscriptionTier = try c.decodeIfPresent(String.self, forKey: .subscriptionTier) ?? SubscriptionTier.unknown.rawValue
        tierRawValue = try c.decodeIfPresent(String.self, forKey: .tierRawValue) ?? ""
        has5TabTrialCode = try c.decodeIfPresent(Bool.self, forKey: .has5TabTrialCode) ?? false
        
        trialExpired = try c.decode(Bool.self, forKey: .trialExpired)
        userStatus = try c.decode(String.self, forKey: .userStatus)
        firstNotificationMessage = try c.decode(String.self, forKey: .firstNotificationMessage)
        secondNotificationMessage = try c.decode(String.self, forKey: .secondNotificationMessage)
        proxyExpiryDate = try c.decodeIfPresent(Date.self, forKey: .proxyExpiryDate)
    }
